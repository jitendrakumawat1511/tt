<div class="container my-5" >
    <div class="tab-wrapper">
        <div class="tabs">
            <div class="tab show-tab" data-tab="tab-1" >tab 1</div>
            <div  class="tab" data-tab="tab-2">tab 2</div>
            <div  class="tab" data-tab="tab-3">tab 3</div>
        </div>
        <div class="tab-content show-tab" id="tab-1">
            <h1>Tab 1</h1>
            Lorem ipsum dolor sit amet consectetur adipisicing elit. Doloremque sequi perferendis perspiciatis, facere officiis labore quis voluptatem itaque quod dolorum explicabo dolor, et nam odio ad? Veritatis, culpa dolor vero nam cum non dolores eligendi sapiente ratione! Nisi quis expedita et facilis consequuntur optio qui totam quisquam debitis, nesciunt magni.
        </div>
        <div class="tab-content" id="tab-2">
            <h1>Tab 2</h1>
            ipsum dolor sit amet consectetur adipisicing elit.lorememque sequi perferendis perspiciatis, facere officiis labore quis voluptatem itaque quod dolorum explicabo dolor, et nam odio ad? Veritatis, culpa dolor vero nam cum non dolores eligendi sapiente ratione! Lorem ipsum dolor sit amet consectetur adipisicing elit. Suscipit consequuntur soluta iure! Praesentium sequi consequatur aperiam deleniti hic sapiente provident tempora distinctio debitis, iste soluta sit minima, quam cumque quaerat. Quae tenetur nobis optio beatae quam quisquam voluptatum eius maiores dignissimos enim ab, ullam eos, a soluta nemo delectus. Ratione?Lorem ipsum dolor sit amet consectetur adipisicing elit. Quaerat, incidunt.Nisi quis expedita et facilis consequuntur optio qui totam quisquam debitis, nesciunt magni.
        </div>
        <div class="tab-content" id="tab-3">
            <h1>Tab 3</h1>
             adipisicing elit. Doloremque sequi perferendis Lorem ipsum dolor sit amet consectetur adipisicing elit. Possimus ipsam autem iste cum maiores recusandae rem quam debitis quas! Veritatis alias odit commodi facilis ipsum culpa ea, tenetur fugiat suscipit illum, dicta voluptatem dolores expedita accusantium dolorum illo quibusdam nesciunt. Veritatis, tempore iure? Explicabo, vitae unde exercitationem architecto deleniti ad? perspiciatis, facere officiis labore quis voluptatem itaque quod dolorum explicabo dolor, et nam odio ad? Veritatis, culpa dolor vero nam cum non dolores eligendi sapiente ratione! Nisi quis expedita et facilis consequuntur optio qui totam quisquam debitis, nesciunt magni.
        </div>
    </div>
</div>